...
def FeaturesImportances(df, colonne, model, model_name):
    ...
    model_FI = model.fit(X_smote, y_smote)
    
    importances1 = pd.Series(index=X_smote.columns[:13],
    ...
    importances2 = pd.Series(index=X_smote.columns[13:26],
    ...
    importances3 = pd.Series(index=X_smote.columns[26:],
